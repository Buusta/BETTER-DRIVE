...
func _process(_delta: float) -> void:
	...
	if mesh_list.size() > 0:
		for i in range(min(chunks_per_frame, mesh_list.size())):
			add_child(mesh_list.pop_front())
...
func _thread_spawn_chunk(chunk_pos):
	...
	call_deferred("_add_mesh_main_thread", mesh_inst)

func _add_mesh_main_thread(m: MeshInstance3D):
