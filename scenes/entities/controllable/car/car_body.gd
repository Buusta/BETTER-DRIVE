...
@export var player_seat: Node3D
@export var exit_point: Node3D

@export var wheel_fl: RaycastWheel
@export var wheel_fr: RaycastWheel
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("move_backward") and active:
		motor_input = -1
...
func _basic_steering_rotation(delta: float):
	...
	if turn_input:
		wheel_fl.rotation.y = clampf(wheel_fl.rotation.y + turn_input * delta * steer_angle,
			deg_to_rad(-tire_max_turn_degrees), deg_to_rad(tire_max_turn_degrees))
		wheel_fr.rotation.y = clampf(wheel_fr.rotation.y + turn_input * delta * steer_angle,
			deg_to_rad(-tire_max_turn_degrees), deg_to_rad(tire_max_turn_degrees))
